...
本番用workflowは `.github/workflows/koo-daily-observe.yml` の `KOO Daily Observe` です。`workflow_dispatch` で手動実行でき、scheduleでは毎日20:40 JST（11:40 UTC）に起動します。実行時間は `KOO_OBSERVE_DURATION_SECONDS=3000` で、20:40〜21:30の監視を想定しています。
